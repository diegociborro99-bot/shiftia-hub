...
function buildNurtureEmail(stage, lead, { appUrl, unsubUrl, escHtml }) {
  ...
  // Personalización: si el lead de auditoría tiene score guardado, el correo
  // del día 2 cita SUS números en vez de un texto genérico.
  const meta = lead.audit_meta && typeof lead.audit_meta === 'object' ? lead.audit_meta : null;
  const hasScore = isAudit && Number.isInteger(lead.audit_score);
  let findingsLine = '';
  if (hasScore && meta) {
    const parts = [];
    if (meta.rest > 0) parts.push(`<strong>${meta.rest}</strong> descanso${meta.rest === 1 ? '' : 's'} entre turnos por debajo del mínimo`);
    if (meta.below_min > 0) parts.push(`<strong>${meta.below_min}</strong> día${meta.below_min === 1 ? '' : 's'} por debajo de tu mínimo de personal`);
    if (meta.weekly > 0) parts.push(`<strong>${meta.weekly}</strong> persona${meta.weekly === 1 ? '' : 's'} sin descanso semanal completo`);
    if (meta.streaks > 0) parts.push(`<strong>${meta.streaks}</strong> racha${meta.streaks === 1 ? '' : 's'} de 3+ noches seguidas`);
    findingsLine = parts.length ? ' Entre otras cosas salieron ' + parts.join(', ') + '.' : '';
  }

  if (stage === 1) {
    return {
      subject: hasScore
        ? `Tu cuadrante puntuó ${lead.audit_score}/100 — por dónde empezar`
        : isAudit ? `¿Le echaste un ojo a tu diagnóstico, ${firstName(lead.name)}?` : 'Tu resultado tiene arreglo (y no es más Excel)',
      ...
      body:
        (hasScore
          ? p(`${nombre}, hace un par de días tu cuadrante puntuó <strong>${lead.audit_score}/100</strong> en la auditoría.${findingsLine} Si ya lo estás arreglando, genial — ignora este correo. Si no, una cosa que vemos a diario:`)
...
// Resumen semanal interno: el pulso del embudo en un correo de lunes.
// stats es un objeto plano ya calculado (las consultas viven en server.js).
const TOOL_LABELS = {
  auditoria: 'Auditorías de cuadrante',
  plantilla: 'Plantillas Excel',
  equidad: 'Calculadora de equidad',
  descanso: 'Calculadora de descansos'
};

function buildWeeklyReportEmail(stats) {
  const row = (label, value, strong) =>
    `<tr><td style="padding:9px 0;color:#4a4a47;font-size:14px;border-bottom:1px solid #ece9e2;">${label}</td>` +
    `<td style="padding:9px 0;color:#0e0f0f;font-size:14px;text-align:right;border-bottom:1px solid #ece9e2;font-variant-numeric:tabular-nums;${strong ? 'font-weight:700;' : ''}">${value}</td></tr>`;

  const toolRows = Object.entries(stats.byTool || {})
    .sort((a, b) => b[1] - a[1])
    .map(([tool, n]) => row('· ' + (TOOL_LABELS[tool] || tool), n))
    .join('');

  const quiet = (stats.totalLeads || 0) === 0 && (stats.bookings || 0) === 0;

  return {
    subject: `Resumen semanal Shiftia · ${stats.totalLeads || 0} leads · ${stats.bookings || 0} llamadas (${stats.weekLabel})`,
    preheader: 'El pulso del embudo de la semana, en 10 segundos.',
    headline: 'Tu semana en Shiftia',
    body:
      `<p style="margin:0 0 18px;color:#1a1a1a;line-height:1.6;font-size:15px;">Semana del ${stats.weekLabel}:</p>` +
      `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="border-top:1px solid #ece9e2;">` +
      row('<strong>Leads nuevos</strong>', stats.totalLeads || 0, true) +
      toolRows +
      row('<strong>Llamadas agendadas</strong>', stats.bookings || 0, true) +
      row('Leads convertidos (total en secuencia que agendó o se registró)', stats.converted || 0) +
      row('En secuencia de seguimiento ahora', stats.inSequence || 0) +
      row('Bajas acumuladas', stats.unsubscribed || 0) +
      `</table>` +
      (quiet
        ? `<p style="margin:18px 0 0;color:#8a6220;font-size:14px;line-height:1.6;">Semana a cero — si no es agosto, toca revisar que los formularios y el tráfico están vivos.</p>`
        : `<p style="margin:18px 0 0;color:#7a766f;font-size:13px;line-height:1.6;">Los leads con 🔥 en tu bandeja son los prioritarios. Este resumen se envía cada lunes por la mañana.</p>`)
  };
}
