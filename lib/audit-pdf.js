...
const REST_STYLE = { bg: '#f2f0ed', fg: '#9a9891', label: 'Libre' };
// Paleta para códigos que no son M/T/N: una tienda usa A (apertura) y C
// (cierre), una fábrica puede usar números. Todos tienen que verse.
const PALETTE = [
  { bg: '#dcefe8', fg: '#1a6b5a' },
  { bg: '#dfe9f6', fg: '#1a5a96' },
  { bg: '#e7e2f6', fg: '#4a3a8a' },
...
const REST_CODES = new Set(['L', 'D', 'V', 'LD', 'DES']);

// Construye el diccionario de estilos y etiquetas a partir de los turnos que
// aparecen de verdad en el cuadrante, respetando el nombre que les dé el
// cliente. Se hace una sola vez por informe para que las dos parrillas y la
// leyenda usen exactamente los mismos colores.
function buildShiftStyles(schedule, fix) {
  const map = new Map();
  const order = [];
  ...
  for (const d of defs) {
    ...
    const rest = d.is_rest || (!d.start && !d.end);
    map.set(code, { rest, hours: d.start && d.end ? `${d.start}–${d.end}` : null, label: d.label || null });
    if (!rest) order.push(code);
  }
  // Códigos vistos en los turnos que no estaban definidos
  for (const sched of [schedule, fix && fix.schedule]) {
    for (const w of ((sched && sched.workers) || [])) {
      for (const sh of (w.shifts || [])) {
        const code = String((sh && sh.code) || '').toUpperCase();
        if (!code || map.has(code)) continue;
        const rest = REST_CODES.has(code);
        map.set(code, { rest, hours: null, label: null });
        if (!rest) order.push(code);
      }
    }
  }
  ...
  order.forEach((code, i) => {
    ...
    const known = SHIFT_STYLE[code];
    const pal = known || PALETTE[i % PALETTE.length];
    styles.set(code, {
      bg: pal.bg, fg: pal.fg,
      label: info.label || (known && known.label) || code,
      hours: info.hours
    });
  });
  for (const [code, info] of map) {
    if (styles.has(code)) continue;
    styles.set(code, { bg: REST_STYLE.bg, fg: REST_STYLE.fg, label: info.label || (SHIFT_STYLE[code] && SHIFT_STYLE[code].label) || 'Libre', hours: null, rest: true });
  }
  return { styles, workCodes: order };
}
...
// mode: 'audit' (el cliente mandó cuadrante) | 'proposal' (no mandó nada y se
// le construye uno con lo que contó por escrito). Cambia el marco, no los datos.
async function buildAuditPdf({ metrics, summary, lead, generatedAt, schedule, fix, mode, assumptions }) {
  const isProposal = mode === 'proposal';
  const { styles: SHIFT_STYLES, workCodes: WORK_CODES } = buildShiftStyles(schedule, fix);
  const shiftStyle = (code) => SHIFT_STYLES.get(String(code || '').toUpperCase()) || REST_STYLE;
  let qrBuffer = null;
  try {
  ...
  return new Promise((resolve, reject) => {
    ...
    const m = metrics;
    // El resumen llega como prosa (lo que devuelve writeSummary) o como
    // {headline, paragraphs}. Se normaliza a lo segundo: el primer párrafo
    // hace de titular y el resto de cuerpo.
    const sum = (() => {
      if (!summary) return { headline: null, paragraphs: [] };
      if (typeof summary === 'string') {
        const parts = summary.split(/\n\s*\n/).map(x => x.trim()).filter(Boolean);
        return { headline: parts[0] || null, paragraphs: parts.slice(1) };
      }
      return {
        headline: summary.headline || null,
        paragraphs: Array.isArray(summary.paragraphs) ? summary.paragraphs.map(String).filter(Boolean) : []
      };
    })();
    ...
    function coverageStrip(x, y, w, dates, sched, opts) {
      ...
      const codes = (WORK_CODES.length ? WORK_CODES : ['M', 'T', 'N'])
        .filter(c => (sched.workers || []).some(wk => (wk.shifts || []).some(sh => String(sh.code).toUpperCase() === c)))
        .slice(0, 5);
      if (!codes.length) return y;

      // La tarjeta se dibuja aquí, con la altura justa: si la calculaba quien
      // llama, se tragaba el título de la sección siguiente.
      card(x, y - 6, w, codes.length * rowH + 34, { bg: '#fdfcfa' });
    ...
    function legend(x, y, codes, extra) {
      ...
      for (const code of codes) {
        ...
        const hours = st.hours || code.hours;
        const text = hours ? `${st.label} ${hours}` : st.label;
    ...
    const shiftCodes = (WORK_CODES.length ? WORK_CODES : ['M', 'T', 'N'])
      .slice(0, 5).map(code => ({ code, hours: null }))
      .concat([{ code: 'L', hours: null }]);
    ...
    let y = isProposal
      ? header(
        `Propuesta de cuadrante · ${lead.sector || 'régimen general'}`,
        'Un cuadrante', 'con tus condiciones',
        `${cut(lead.cleanName, 40)} · ${periodTxt} · construido con lo que nos contaste, sin adjuntar ningún documento`
      )
      : header(
    ...
    y = kpiRow(y, [
      { label: isProposal ? 'Puntuación' : 'Puntuación global', value: m.score, unit: '/100', sub: m.score_label, dark: true },
      { label: 'Personas', value: m.workers_count, sub: `${m.total_shifts} turnos ${isProposal ? 'repartidos' : 'analizados'}` },
      ...
      m.nights.total > 0
        ...
        : { label: 'Reparto de findes', value: m.weekends.range <= 1 ? 'Justo' : m.weekends.range <= 2 ? 'Mejorable' : 'Desigual', color: m.weekends.range <= 1 ? C.green : m.weekends.range <= 2 ? C.amber : C.red, sub: `diferencia máx-mín: ${m.weekends.range}` }
    ]);

    const current = schedule ? buildRosterRows(schedule, null) : null;
    if (current && current.dates.length && current.rows.length) {
      y = sectionLabel(M, y, isProposal
        ? `La propuesta · ${current.rows.length} personas × ${current.dates.length} días, rotación completa`
        : `El cuadrante que nos enviaste · ${current.rows.length} personas × ${current.dates.length} días`);
      const gridBottom = rosterGrid(M, y, CW, current.dates, current.rows, { totals: true, maxH: H - y - 190 });
      y = gridBottom + 10;
      y = legend(M, y, shiftCodes, isProposal ? [] : [{ color: C.red, label: 'celda en conflicto' }]) + 8;
      y = sectionLabel(M, y, isProposal
        ? 'Cobertura día a día · exactamente la que pediste, todos los días'
        : 'Cobertura día a día · lo que la corrección NO toca');
      y = coverageStrip(M, y + 2, CW, current.dates, schedule, { totals: true }) + 8;
      ...
      if (sum.headline) {
        doc.font('serif').fontSize(15).fillColor(C.ink)
          .text(sum.headline, M, y, { width: CW * 0.62 });
        y = doc.y + 8;
      }
      for (const p2 of sum.paragraphs.slice(0, 3)) {
    ...
    doc.addPage();
    y = isProposal
      ? header('Comprobación', 'Lo que esta propuesta', 'cumple',
        'La propuesta de la página anterior pasa por el mismo motor de reglas con el que auditamos los cuadrantes ajenos.')
      : header('Diagnóstico', 'Lo que hemos', 'encontrado',
    ...
    let ly = y;
    ly = sectionLabel(M, ly, isProposal ? 'Cómo se ha construido' : 'Resumen', colW2);
    if (sum.headline) {
      doc.font('sans-bold').fontSize(9.5).fillColor(C.ink)
        .text(sum.headline, M, ly, { width: colW2, lineGap: 1.5 });
      ly = doc.y + 5;
    }
    for (const p of sum.paragraphs.slice(0, 3)) {
      doc.font('sans').fontSize(8).fillColor(C.muted).text(p, M, ly, { width: colW2, lineGap: 1.5 });
      ly = doc.y + 4;
    }

    if (isProposal && Array.isArray(assumptions) && assumptions.length) {
      ly += 6;
      ly = sectionLabel(M, ly, 'Lo que hemos dado por supuesto · corrígenos y lo rehacemos', colW2);
      for (const a of assumptions.slice(0, 8)) {
        doc.font('sans').fontSize(7.8).fillColor(C.muted).text('·  ' + a, M, ly, { width: colW2, lineGap: 1.2 });
        ly = doc.y + 3;
      }
    }

    // Con turnos sin noche (comercio, oficina) el reparto de noches es una
    // lista de ceros: lo que importa ahí es quién carga con los fines de semana.
    ly += 6;
    const useNights = m.nights.total > 0;
    const dist = useNights
      ? { rows: m.nights.per_worker.map(p => ({ name: p.name, v: p.nights })), over: m.nights.overloaded, max: m.nights.max, jain: m.nights.jain, mean: m.nights.mean, label: 'Reparto de noches' }
      : { rows: (m.weekends.per_worker || []).map(p => ({ name: p.name, v: p.weekend_shifts })), over: m.weekends.overloaded || [], max: Math.max(1, ...(m.weekends.per_worker || []).map(p => p.weekend_shifts)), jain: m.weekends.jain, mean: m.weekends.mean, label: 'Reparto de fines de semana' };
    ly = sectionLabel(M, ly, `${dist.label} · media ${comma(dist.mean)} · índice de equidad ${comma(dist.jain)}`, colW2);
    const maxN = Math.max(1, dist.max);
    for (const p of dist.rows.slice(0, 10)) {
      const over = dist.over.includes(p.name);
      doc.font('sans').fontSize(7.5).fillColor(over ? C.red : C.ink)
        .text(cut(p.name, 22), M, ly, { width: 96, lineBreak: false });
      bar(M + 100, ly + 1.5, colW2 - 100 - 52, p.v, maxN, { color: over ? C.red : C.teal });
      doc.font('sans-bold').fontSize(7.5).fillColor(over ? C.red : C.ink)
        .text(String(p.v), M + colW2 - 48, ly, { width: 22, align: 'right', lineBreak: false });
    ...
    if (fichasY < H - 120) {
      ...
      if (per > 0) {
        ...
        names.forEach((name, i) => {
          ...
          const rows = (m.nights.total > 0 ? [['Noches', String(nightsByName.get(name) ?? 0)]] : [])
            .concat([
    ...
    doc.addPage();
    y = isProposal
      ? header('Siguiente paso', 'Esto es un punto', 'de partida',
        'La propuesta de estas páginas la ha construido Shiftia en segundos con cuatro frases tuyas. Con tus datos reales, el salto es mayor.')
      : header('Siguiente paso', 'Esto es una foto.', 'Shiftia lo mantiene así',
