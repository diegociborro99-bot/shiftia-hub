...
function buildAuditPdf({ metrics, summary, lead, generatedAt }) {
  return new Promise((resolve, reject) => {
    ...
    paper();
    try {
      if (fs.existsSync(LOGO_PATH)) doc.image(LOGO_PATH, M, M - 8, { height: 34 });
      else throw new Error('no logo');
    } catch (e) {
      doc.font('serif-italic').fontSize(26).fillColor(C.ink).text('Shiftia', M, M - 4);
    }
    doc.font('sans').fontSize(9).fillColor(C.subtle)
      .text('Auditoría de cuadrante · Informe confidencial', M, M + 2, { width: CW, align: 'right' });
    ...
    chips.forEach((c, i) => {
      ...
      doc.roundedRect(x, chipY, chipW, 56, 8).lineWidth(0.8).fillAndStroke(C.cardBg, C.border);
      doc.font('sans').fontSize(7.5).fillColor(C.subtle).text(c.label.toUpperCase(), x + 12, chipY + 11, { width: chipW - 24, characterSpacing: 0.8 });
      doc.font('serif').fontSize(21).fillColor(c.color).text(c.value, x + 12, chipY + 24, { width: chipW - 24 });
    });
    doc.y = chipY + 70;
    ...
    m.nights.per_worker.slice(0, 20).forEach(x => {
      ...
      tableRow([
        { text: x.name, w: CW * 0.55 },
        { text: String(x.nights), w: CW * 0.2, align: 'right' },
        { text: over ? 'Sobrecarga' : (x.nights < m.nights.mean - 1 ? 'Por debajo de la media' : 'En media'), w: CW * 0.25, align: 'right', color: over ? C.red : C.subtle, size: 8.5 }
      ]);
    });
    ...
    doc.moveDown(1.4);
    const ctaY = doc.y;
    doc.roundedRect(M, ctaY, CW, 86, 10).fill(C.ink);
    doc.font('serif-italic').fontSize(16).fillColor('#faf9f6')
      .text('¿Y si tu próximo cuadrante saliera ya así de auditado?', M + 24, ctaY + 18, { width: CW - 48 });
    doc.font('sans').fontSize(9.5).fillColor('#d8d6cf')
      .text('La IA de Shiftia genera la planilla respetando descansos, equidad y tu convenio. Te lo enseñamos con tus datos en 15 minutos: www.shiftia.es/#contact · info@shiftia.es', M + 24, ctaY + 44, { width: CW - 48, lineGap: 2 });
    doc.y = ctaY + 100;
