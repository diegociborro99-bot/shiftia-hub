...
async function buildAuditPdf({ metrics, summary, lead, generatedAt }) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (m.weekly_rest_issues.length) {
      sectionTitle('Descanso semanal (art. 37.1 ET: 36 h continuas)');
      doc.font('sans').fontSize(9).fillColor(C.muted).text(
        'Personas sin ningún descanso continuo de 36 h en un tramo superior a 14 días (el art. 37.1 ET permite acumular el descanso semanal en periodos de hasta 14 días):',
        M, doc.y, { width: CW, lineGap: 2 });
      doc.moveDown(0.5);
      m.weekly_rest_issues.slice(0, 12).forEach(x => {
        ensureSpace(18);
        doc.font('sans').fontSize(9.5).fillColor(C.red)
          .text(`${x.worker}: ${x.days_without_weekly_rest} días sin descanso semanal completo`, M, doc.y, { width: CW });
        doc.moveDown(0.25);
      });
    }

    // ===== Días de trabajo consecutivos =====
    if (m.consecutive_work_runs.length) {
      sectionTitle('Tramos de 7 o más días de trabajo sin librar');
      m.consecutive_work_runs.slice(0, 12).forEach(x => {
        ensureSpace(18);
        doc.font('sans').fontSize(9.5).fillColor(C.ink)
          .text(`${x.worker}: ${x.length} días seguidos trabajando (hasta ${x.end_date})`, M, doc.y, { width: CW });
        doc.moveDown(0.25);
      });
    }

    // ===== Cobertura =====
    const FRANJA = { manana: 'Mañana', tarde: 'Tarde', noche: 'Noche' };
    if (m.coverage.source === 'minimos_declarados') {
      sectionTitle('Cobertura frente a tu mínimo declarado');
      const declared = m.coverage.declared_minimums || {};
      const declaredTxt = ['manana', 'tarde', 'noche'].filter(k => declared[k]).map(k => `${FRANJA[k].toLowerCase()}: ${declared[k]}`).join(' · ');
      doc.font('sans').fontSize(9).fillColor(C.muted).text(
        `Comparación contra las necesidades de personal que describiste en el formulario, interpretadas junto con tu cuadrante como mínimo por franja (${declaredTxt}). Si la interpretación no encaja, respóndenos al email y lo recalculamos.`,
        M, doc.y, { width: CW, lineGap: 2 });
      doc.moveDown(0.5);
      if (!m.coverage.below_minimum.length) {
        doc.font('sans-bold').fontSize(10).fillColor(C.tealDark).text('Todos los días del periodo cumplen el mínimo declarado. ✔', M, doc.y, { width: CW });
        doc.moveDown(0.4);
      } else {
        tableRow([
          { text: 'Fecha', w: CW * 0.25, bold: true, color: C.subtle, size: 8.5 },
          { text: 'Franja', w: CW * 0.25, bold: true, color: C.subtle, size: 8.5 },
          { text: 'Personas', w: CW * 0.25, bold: true, color: C.subtle, size: 8.5, align: 'right' },
          { text: 'Tu mínimo', w: CW * 0.25, bold: true, color: C.subtle, size: 8.5, align: 'right' }
        ]);
        m.coverage.below_minimum.slice(0, 20).forEach(b => {
          tableRow([
            { text: b.date, w: CW * 0.25 },
            { text: FRANJA[b.franja] || b.franja, w: CW * 0.25 },
            { text: String(b.count), w: CW * 0.25, align: 'right', color: b.count === 0 ? C.red : C.amber },
            { text: String(b.minimum), w: CW * 0.25, align: 'right', color: C.muted }
          ]);
        });
        if (m.coverage.below_minimum.length > 20) {
          doc.font('sans').fontSize(8.5).fillColor(C.subtle).text(`…y ${m.coverage.below_minimum.length - 20} días más por debajo del mínimo.`, M, doc.y + 2);
          doc.moveDown(0.5);
        }
      }
    } else if (m.coverage.evaluated && (m.coverage.empty_slots.length || m.coverage.low_slots.length)) {
      sectionTitle('Cobertura aparente por turno');
      doc.font('sans').fontSize(9).fillColor(C.muted).text(
        'Estimación deducida del propio cuadrante (días muy por debajo de lo habitual en cada turno). Si nos cuentas en el formulario cuánta gente necesitas por turno, este apartado se audita contra tu necesidad real en vez de estimarse.',
        M, doc.y, { width: CW, lineGap: 2 });
      doc.moveDown(0.5);
      m.coverage.empty_slots.slice(0, 10).forEach(x => {
        ensureSpace(18);
        doc.font('sans').fontSize(9.5).fillColor(C.red)
          .text(`${x.date}: nadie asignado al turno ${x.code} (lo habitual son ${x.habitual})`, M, doc.y, { width: CW });
        doc.moveDown(0.25);
      });
      m.coverage.low_slots.slice(0, 10).forEach(x => {
        ensureSpace(18);
        doc.font('sans').fontSize(9.5).fillColor(C.amber)
          .text(`${x.date}: solo ${x.count} en el turno ${x.code} (lo habitual son ${x.habitual})`, M, doc.y, { width: CW });
        doc.moveDown(0.25);
      });
    }

    // ===== Fines de semana y rotación =====
    if (m.weekends.total >= 4 || m.backward_rotations.total > 0) {
      sectionTitle('Fines de semana y sentido de la rotación');
      if (m.weekends.total >= 4) {
        const wkTop = m.weekends.per_worker[0];
        doc.font('sans').fontSize(9.5).fillColor(C.ink).text(
          `Turnos en sábado/domingo: ${m.weekends.total} (media ${comma(m.weekends.mean)} por persona · equidad de Jain ${comma(m.weekends.jain)}/1).` +
          (m.weekends.overloaded.length ? ` Cargan más fines de semana de la cuenta: ${m.weekends.overloaded.join(', ')}.` : wkTop ? ` Quien más acumula: ${wkTop.name} (${wkTop.weekend_shifts}).` : ''),
          M, doc.y, { width: CW, lineGap: 2 });
        doc.moveDown(0.4);
      }
      if (m.backward_rotations.total > 0) {
        doc.font('sans').fontSize(9.5).fillColor(C.ink).text(
          `Rotaciones antihorarias (p. ej. noche→tarde o tarde→mañana en días consecutivos): ${m.backward_rotations.total}. ` +
          `Más afectados: ${m.backward_rotations.per_worker.slice(0, 4).map(x => `${x.worker} (${x.count})`).join(', ')}. ` +
          'La rotación regresiva es la que más rompe el ritmo circadiano; la recomendación ergonómica es rotar M→T→N.',
          M, doc.y, { width: CW, lineGap: 2 });
        doc.moveDown(0.4);
      }
    }
    ...
    if (m.roles.leaders.length || m.roles.expected_but_not_identified) {
      sectionTitle('Cobertura de encargados / responsables');
      if (m.roles.leaders.length) {
        doc.font('sans').fontSize(9.5).fillColor(C.ink).text(
          `Responsables identificados en el cuadrante: ${m.roles.leaders.join(', ')}.`,
          M, doc.y, { width: CW, lineGap: 2 });
        doc.moveDown(0.35);
        if (m.roles.days_without_leader.length) {
          doc.font('sans').fontSize(9.5).fillColor(C.red).text(
            `Días del periodo sin ningún responsable trabajando: ${m.roles.days_without_leader.slice(0, 12).join(', ')}${m.roles.days_without_leader.length > 12 ? ` …y ${m.roles.days_without_leader.length - 12} más` : ''}.`,
            M, doc.y, { width: CW, lineGap: 2 });
        } else {
          doc.font('sans-bold').fontSize(10).fillColor(C.tealDark).text('Todos los días del periodo tienen al menos un responsable. ✔', M, doc.y, { width: CW });
        }
        doc.moveDown(0.4);
      } else {
        doc.font('sans').fontSize(9.5).fillColor(C.muted).text(
          'Indicaste que el cuadrante incluye encargados, pero el documento no permite identificar su categoría con garantías, así que este punto no se ha evaluado (preferimos decírtelo a inventarlo). Si la categoría aparece en otra columna u hoja, reenvíanos el archivo completo respondiendo al email.',
          M, doc.y, { width: CW, lineGap: 2 });
        doc.moveDown(0.4);
      }
    }
