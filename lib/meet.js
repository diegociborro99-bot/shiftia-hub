...
async function createMeetEvent({
  ...
  if (!meetLink) {
    // El evento ya existe en el calendario: sin enlace no sirve y nadie lo
    // referenciará (el caller descarta el eventId al fallar) — se borra para
    // no dejar huérfanos. Best-effort: el error original es el que importa.
    if (ev.id) {
      await fetchImpl(`${CALENDAR_API}/calendars/${encodeURIComponent(calendarId)}/events/${encodeURIComponent(ev.id)}?sendUpdates=none`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${accessToken}` },
        signal: AbortSignal.timeout(timeoutMs)
      }).catch(() => {});
    }
    throw new Error('Evento creado pero sin enlace de Meet');
  }
