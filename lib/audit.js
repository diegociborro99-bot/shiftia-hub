...
function analyzeSchedule(schedule, opts) {
  ...
  // Puntuación global 0-100 (determinista): 40 pts equidad (índice de Jain),
  // 40 pts descansos (penaliza incumplimientos duros más que zonas grises),
  // 20 pts rachas de noches.
  const hard = restViolations.filter(v => v.severity === 'incumplimiento').length;
  const soft = restViolations.length - hard;
  const restFactor = Math.max(0, 1 - (hard * 0.15 + soft * 0.05));
  const streakFactor = Math.max(0, 1 - nightStreaks.length * 0.25);
  const score = Math.max(3, Math.round(40 * jainIndex(nightCounts) + 40 * restFactor + 20 * streakFactor));
