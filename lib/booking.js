...
// Horas cerradas de un día concreto por un cierre parcial —"el 30 a partir de
// las 12"—, frente a los cierres de día entero que van por blocked_dates.
// `from` entra en el cierre y `to`, si se da, queda fuera; sin `from` se cierra
// desde el principio del día y sin `to`, hasta el final. Un tramo mal escrito
// se ignora: más vale ofrecer una hora de más que tumbar el arranque.
function partialClosureSlots(dateStr, partials, slots) {
  const minutos = (t) => {
    const m = /^(\d{1,2}):([0-5]\d)$/.exec(String(t || ''));
    return m ? Number(m[1]) * 60 + Number(m[2]) : null;
  };
  const fuera = new Set();
  for (const cierre of partials || []) {
    if (!cierre || cierre.date !== dateStr) continue;
    const desde = cierre.from === undefined ? 0 : minutos(cierre.from);
    const hasta = cierre.to === undefined ? 24 * 60 : minutos(cierre.to);
    if (desde === null || hasta === null || desde >= hasta) continue;
    for (const t of slots) {
      const m = minutos(t);
      if (m !== null && m >= desde && m < hasta) fuera.add(t);
    }
  }
  return fuera;
}
